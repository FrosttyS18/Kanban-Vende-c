...
export function subscribeBoardRealtimeWithOptions(boardId: string, onChange: RealtimeCallback, options?: RealtimeSubscribeOptions): () => void {
  ...
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'boards', filter: `id=eq.${boardId}` }, (payload) => {
    onChange({ table: 'boards', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'lists', filter: `board_id=eq.${boardId}` }, (payload) => {
    ...
    onChange({ table: 'lists', eventType: typedPayload.eventType ?? 'UPDATE', new: typedPayload.new, old: typedPayload.old })
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'labels', filter: `board_id=eq.${boardId}` }, (payload) => {
    onChange({ table: 'labels', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'board_members', filter: `board_id=eq.${boardId}` }, (payload) => {
    onChange({ table: 'board_members', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'board_share_links', filter: `board_id=eq.${boardId}` }, (payload) => {
    onChange({ table: 'board_share_links', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'cards' }, (payload) => {
    if (isScopedCardEvent(payload as RealtimePayload)) {
      onChange({ table: 'cards', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
    }
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'card_labels' }, (payload) => {
    if (isScopedCardChildEvent(payload as RealtimePayload)) {
      onChange({ table: 'card_labels', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
    }
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'card_members' }, (payload) => {
    if (isScopedCardChildEvent(payload as RealtimePayload)) {
      onChange({ table: 'card_members', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
    }
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'card_links' }, (payload) => {
    if (isScopedCardChildEvent(payload as RealtimePayload)) {
      onChange({ table: 'card_links', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
    }
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'card_activities' }, (payload) => {
    if (isScopedCardChildEvent(payload as RealtimePayload)) {
      onChange({ table: 'card_activities', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
    }
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'checklists' }, (payload) => {
    ...
    onChange({ table: 'checklists', eventType: typedPayload.eventType ?? 'UPDATE', new: typedPayload.new, old: typedPayload.old })
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'checklist_items' }, (payload) => {
    ...
    if (hasChecklistInScope) {
      onChange({ table: 'checklist_items', eventType: typedPayload.eventType ?? 'UPDATE', new: typedPayload.new, old: typedPayload.old })
    }
  })
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'notifications' }, (payload) => {
    ...
    if (userId === options.currentUserId) {
      onChange({ table: 'notifications', eventType: payload.eventType ?? 'UPDATE', new: payload.new, old: payload.old })
    }
  })
